 ...
 * which occurs with the unmodified formula.
 */
...
#define SlzBRDFData LagrangeGGXParams


/**
 * Get struct containing most of the parameters needed to calculate a GGX specular highlight using the float16-optimized
 * Lagrange identity version of the formula. 
 *
 ...
 * @return struct containing the parameters needed for the GGX specular formula
 */
LagrangeGGXParams GetLagrangeGGXParams(const half3 normal, const half3 viewDir, const half3 lightDir)
{
 ...
 * @return GGX normal distribution value
 */
float GgxNdfFullFloat(float NoH, float roughness)
{
 ...
 */
half SLZFusedVisFresnel(half LoH, half roughness)
{
    half LoH2 = LoH * LoH;
 ...
 */ 
half3 SLZSchlickFresnel(half LoH, half3 reflectance0, half3 reflectance90)
{
 ...
 */
half3 SLZBRDFSpecFP16FusedVF(LagrangeGGXParams ggx, half roughness, half3 reflectance)
{
    half NDF = GgxNdfLagrange(ggx.NoH, ggx.NxH2, roughness);
    half VF  = SLZFusedVFMobile(ggx.LoH, roughness);
 ...
 */
half3 SLZBRDFSpecFP16(LagrangeGGXParams ggx, half NoV, half roughness, const half3 reflectance0, const half3 reflectance90 = half3(1,1,1))
{
	...
	half3 F = SLZSchlickFresnel(LoH, reflectance0, reflectance90);

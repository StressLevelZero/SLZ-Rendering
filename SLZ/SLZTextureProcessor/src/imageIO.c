...
#define LIB_TIF TXP_IMGLIB_TIFF
...
typedef struct tiff_data
{
    TIFF* tiff;

    uint16_t channels;
    uint16_t channelSize;
    uint16_t channelFormat;
    uint16_t orientation;
    uint16_t compression;
    uint16_t planarConfig;
    
    uint16_t extraChannels;
    uint16_t* extraChannelsDef;
} tiff_data;
...
static TXPErrorCode GetLibraryFileDataTIFF(void** ild, const PathString* path)
{
    TXPErrorCode err = TXP_RETURN_SUCCESS;
    tiff_data* data = (tiff_data*)malloc(sizeof(tiff_data));
    if (data == NULL) { err = TXP_RETURN_ALLOC_FAILED; goto exit; }
    *data = (tiff_data){};

#if defined(PLATFORM_WINDOWS)
    data->tiff = TIFFOpenW(path->s, "r");
#else
    data->tiff = TIFFOpen(path->s, "r");
#endif
...
TXPErrorCode GetLibraryFileData(void** ild, ImageLibrary lib, const PathString* path)
{
    ...
    case (TXP_IMGLIB_TIFF): err = GetLibraryFileDataTIFF(ild, path); break;
    default: 
...
static void DisposeLibraryFileDataTIFF(ImageIoHandler* imageIO)
{
    tiff_data* data = (tiff_data*)imageIO->ILD;
    if (data != NULL)
    {
        if (data->tiff != NULL) TIFFClose(data->tiff);
        free(data);
    }
    imageIO->ILD = NULL;
}
...
void DisposeLibraryFileData(ImageIoHandler* imageIO)
{
    ...
    case (TXP_IMGLIB_TIFF): DisposeLibraryFileDataTIFF(imageIO);    break;
    default: break;
    }
}
...
static TXPErrorCode GetImageInfoTIFF(ImageIoHandler* imageIO)
{
    TXPErrorCode err = TXP_RETURN_SUCCESS;
    tiff_data* data = (tiff_data*)imageIO->ILD;
    TIFF* tiff = data->tiff;

    imageIO->channelFormat = CHANNEL_FMT_UNKNOWN;
    imageIO->width = 0;
    imageIO->height = 0;
    imageIO->channelCount = 0;
    ...
    if (tiff != NULL)
    {
        ...
        TIFFGetField(tiff, TIFFTAG_SAMPLESPERPIXEL, &(data->channels	 ));
        TIFFGetField(tiff, TIFFTAG_BITSPERSAMPLE,	&(data->channelSize	 ));
        TIFFGetField(tiff, TIFFTAG_SAMPLEFORMAT,	&(data->channelFormat));
        TIFFGetField(tiff, TIFFTAG_PLANARCONFIG,	&(data->planarConfig ));
        TIFFGetField(tiff, TIFFTAG_ORIENTATION,		&(data->orientation  ));
        if (data->channels > 3 && data->channels < 8)
        {
            TIFFGetField(tiff, TIFFTAG_EXTRASAMPLES, &(data->extraChannels), &(data->extraChannelsDef));
        }
        uint16_t extraChannelsDef[4] = { 0,0,0,0 };
        for (int i = 0; i < data->extraChannels && i < 4; i++)
        {
            extraChannelsDef[i] = data->extraChannelsDef[i];
        }

        DebugLog(UNITY_LOG_LEVEL_LOG, 
            "TIFF file info:\n"
            "size: %d x %d\n"
            "channels: %d\n"
            "bits per channel: %d\n"
            "planar config: %d\n"
            "orientation: %d\n"
            "extra channels: %d, types ( %d %d %d %d )\n"
            , 
            width, height,
            data->channels,
            data->channelSize,
            data->planarConfig,
            data->orientation,
            data->extraChannels, extraChannelsDef[0], extraChannelsDef[1], extraChannelsDef[2], extraChannelsDef[3]
            );

        imageIO->width = width;
        imageIO->height = height;
        imageIO->channelCount = data->channels < 5 ? data->channels : 4;
        imageIO->yFlip = data->orientation != ORIENTATION_BOTLEFT  && 
                         data->orientation != ORIENTATION_LEFTBOT  && 
                         data->orientation != ORIENTATION_BOTRIGHT &&
                         data->orientation != ORIENTATION_RIGHTBOT;

        imageIO->channelFormat = CHANNEL_FMT_FIXED8;
#if 0
        switch (data->channelFormat)
        {
        case 0:
        case SAMPLEFORMAT_UINT:
            switch (data->channelSize)
            {
            case 8:  imageIO->channelFormat = CHANNEL_FMT_FIXED8; imageIO->channelCount = 4; break;
            case 16: imageIO->channelFormat = CHANNEL_FMT_FIXED16; break;
            default:
                imageIO->channelFormat = CHANNEL_FMT_UNKNOWN;
                DebugLog(UNITY_LOG_LEVEL_ERROR, "Cannot read TIFF file, uses unhandled %d bit integer data type", data->channelSize);
                return TXP_RETURN_UNSUPPORTED_TIFF_INVALID_CHANNEL_FORMAT;
            }
            break;
        case SAMPLEFORMAT_IEEEFP:
            switch (data->channelSize)
            {
            case 16: imageIO->channelFormat = CHANNEL_FMT_FLOAT16;    break;
            case 32: imageIO->channelFormat = CHANNEL_FMT_FLOAT32;   break;
            default:
                imageIO->channelFormat = CHANNEL_FMT_UNKNOWN;
                DebugLog(UNITY_LOG_LEVEL_ERROR, "Cannot read TIFF file, uses unhandled %d bit floating point data type", data->channelSize);
                return TXP_RETURN_UNSUPPORTED_TIFF_INVALID_CHANNEL_FORMAT;
            }
            break;
        default:
            DebugLog(UNITY_LOG_LEVEL_ERROR, "Cannot read TIFF file, uses unhandled channel data type %d", data->channelFormat);
            imageIO->channelFormat = CHANNEL_FMT_UNKNOWN;
            return TXP_RETURN_UNSUPPORTED_TIFF_INVALID_CHANNEL_FORMAT;
        }
#endif
    ...
    {
        DebugLog(UNITY_LOG_LEVEL_ERROR, "Cannot read TIFF file, file handle is NULL");
        err = TXP_RETURN_FILE_OPEN_FAILED;
    }
    return err;
}
...
TXPErrorCode GetImageInfo(ImageIoHandler* imageIO)
{
    ...
    case (TXP_IMGLIB_TIFF): return GetImageInfoTIFF(imageIO); break;
    default: 
...
static TXPErrorCode IOReadImageTIFF(ImageIoHandler* imageIO)
{
    TXPErrorCode err = TXP_RETURN_SUCCESS;
    ...
    int channelSize = ChannelFmtToBytes(imageIO->channelFormat);
    size_t bufferSize = imageIO->width * imageIO->height * (size_t)(channelSize * imageIO->channelCount);
    imageIO->imageBuffer = _TIFFmalloc(bufferSize);
    int returnCode = TIFFReadRGBAImage(tiff, imageIO->width, imageIO->height, (uint32_t*)imageIO->imageBuffer, 0);
    if (returnCode == 0)
    {
        DebugLog(UNITY_LOG_LEVEL_ERROR, "LibTiff could not read tiff file");
    }
    return returnCode == 0 ? TXP_RETURN_IMG_LIBRARY_FAILED : TXP_RETURN_SUCCESS;
}
...
TXPErrorCode IOReadImage(ImageIoHandler* imageIO)
{
    ...
    case (TXP_IMGLIB_TIFF): return IOReadImageTIFF(imageIO);
    default: break;
    }
...
static void DisposeImageBufferTIFF(ImageIoHandler* imageIO)
{
    if (imageIO != NULL && imageIO->imageBuffer != NULL)
    {
        _TIFFfree(imageIO->imageBuffer);
    }
}
...
void DisposeImageBuffer(ImageIoHandler* imageIO)
{
    ...
    case (TXP_IMGLIB_TIFF): DisposeImageBufferTIFF(imageIO); break;
    default: break;
    }
}
...
TXPErrorCode SaveToPng(PATH_CHAR* savePath, void* imageBuffer, int width, int height, TxpTextureFormat pixelFormat, int compressionLevel)
{
    ...
    {
    case (FMT_R8):
        ihdr2.color_type = SPNG_COLOR_TYPE_GRAYSCALE;
        ihdr2.bit_depth = 8;
        break;
    case (FMT_R16):
        ihdr2.color_type = SPNG_COLOR_TYPE_GRAYSCALE;
    ...
    case (FMT_RGBA16):
        ...
        ihdr2.bit_depth = 16;
        break;
    default: DebugLog(UNITY_LOG_LEVEL_ERROR, "SPNG writer: invalid input texture format. SPNG can only output 8 or 16 bit R, RGB, or RGBA images");
    ...
    spng_set_option(enc, SPNG_IMG_COMPRESSION_LEVEL, compressionLevel);
    spng_set_ihdr(enc, &ihdr2);
    spng_encode_image(enc, imageBuffer, width * height * TextureFormatSize(pixelFormat), SPNG_FMT_PNG, SPNG_ENCODE_FINALIZE);
    size_t outSize;
    int error;
    void* png = spng_get_png_buffer(enc, &outSize, &error);
    DebugLog(UNITY_LOG_LEVEL_LOG, "SPNG Error Code: %d\n", error);
    ...
    FILE* outPng = NULL;
    FILE_OPEN(outPng, savePath, PATH_LITERAL("wb"));
    if (outPng == NULL)
    {
        DebugLog(UNITY_LOG_LEVEL_ERROR, "SPNG Writer: Failed to open file for write");
        free(png);
        spng_ctx_free(enc);
        return TXP_RETURN_FILE_OPEN_FAILED;
    }
    size_t written = fwrite(png, 1, outSize, outPng);
    if (written != outSize)
    {
        DebugLog(UNITY_LOG_LEVEL_ERROR, "SPNG Writer: Failed to write all bytes to file");
        fclose(outPng);
        free(png);
        spng_ctx_free(enc);
        return TXP_RETURN_GENERAL_FAILURE;
    }
    fclose(outPng);
    free(png);
    spng_ctx_free(enc);
